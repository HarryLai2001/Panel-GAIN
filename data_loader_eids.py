...
def load_ET_data(path, batch_size):
    df = pd.read_excel(path, engine='openpyxl')
    agg = df.groupby(['first_treat', 'year']).agg(y_mean = ("perfdi", np.mean),
                                                  y_std = ("perfdi", np.std))

    df = pd.merge(df, agg, how='left', on=['first_treat', 'year'])
    df['y_norm'] = (df['perfdi'] - df['y_mean']) / (df['y_std'] + 1e-8)
    ...
    for unit, unit_fea in df.groupby('code'):
        d.append(np.expand_dims(unit_fea['d'].to_numpy(), axis=0))
        y.append(np.expand_dims(unit_fea["perfdi"].to_numpy(), axis=0))
