...
df = read.xlsx("res_all.xlsx") %>%
  pivot_longer(cols = -c("first_treat", "year", "time_to_treat"))

groups = split(df, df$first_treat)
...
for(g in names(groups)) {
  ...
  p = ggplot(df, aes(x = year, y = value, group = name, color = name)) +
    ...
    geom_vline(xintercept = as.numeric(g) - 0.5, size = 0.3, linetype = 'dashed', color = 'gray50') + 
    geom_hline(yintercept = 0, size = 0.3, linetype = 'dashed', color = 'gray50') + 
    scale_y_continuous(name = "组别-时间平均干预效应", limits = c(-1, 1), breaks = seq(-1, 1, 0.2)) + 
...
ggsave(glue("plot.png"), width = 14, height = 4)