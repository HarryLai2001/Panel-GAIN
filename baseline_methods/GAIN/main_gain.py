...
from gain import Generator, Discriminator
from trainer_gain import Trainer
...
if __name__ == '__main__':
    ...
    for rep in range(reps_num):
        ...
        generator = Generator(t_dim=T_obs, h_dim=T_obs, num_ffns=4)
    ...
    with pd.ExcelWriter(f"../../result/simu_model1/GAIN/simu1 T_pre={T_pre} reps_res.xlsx", engine='openpyxl') as writer:
    ...
    with pd.ExcelWriter(f"../../result/simu_model2/GAIN/simu2 T_pre={T_pre} reps_res.xlsx", engine='openpyxl') as writer:
        pred_err.to_excel(writer, sheet_name="pred_err", index=False)
        gt_est_df.to_excel(writer, sheet_name="gt_est", index=False)
